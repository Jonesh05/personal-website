...
import { toggleSave } from '@/lib/firestore/interactions'
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const rl = checkRateLimit(`posts:save:${clientKey}`, DEFAULT_PUBLIC_LIMIT)
    if (!rl.allowed) {
    ...
    const result = await toggleSave(id, visitorId, {
      slug: existing.slug,
